...
- **`src/server/model/`**: Data models (Activity, etc.)
- **`src/server/sql.gleam`**: Generated by Squirrel (DO NOT EDIT MANUALLY)

### Database Workflow

1. **Schema changes**: Add SQL files to `priv/migrations/`
2. **Queries**: Add `.sql` files to `src/server/sql/`
3. **Regenerate**: Run `gleam run -m squirrel` to generate type-safe Gleam code in `src/server/sql.gleam`
...
Routes are defined in `src/server/router.gleam`:
- `/` → home page
- `/activities` → lists all activities from database
- `/book/:id` → booking page for specific activity
- `/api/activities` → API endpoint for all activities
- `/api/activities/:id` → API endpoint for specific activity
- `/api/app-config` → API endpoint for app configuration
